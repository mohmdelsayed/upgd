...
class Plotter:
    ...
    def plot_last_n_tasks(self, n_tasks=5):
        for subdir in self.best_runs_path:
            ...
            mean_list = np.array(configuration_list).mean(axis=0)[-n_tasks * self.avg_interval-1:]
            std_list = np.array(configuration_list).std(axis=0)[-n_tasks * self.avg_interval-1:] / np.sqrt(len(seeds))
            plt.plot(mean_list, label=learner_name)
            plt.title(f"First {n_tasks} tasks {self.task_name}")
            plt.fill_between(range(len(mean_list)), mean_list - std_list, mean_list + std_list, alpha=0.1)
            plt.legend()
...
if __name__ == "__main__":
    ...
    # best_runs = BestRun("summer_with_sign_change", "area", "fully_connected_linear", ["sgd", "upgdv2_normalized"]).get_best_run()
    best_runs = BestRun("summer_with_signals_change", "area", "fully_connected_tanh", ["sgd", "upgdv2_normalized"]).get_best_run()

    print(best_runs)
    plotter = Plotter(best_runs, task_name="summer_with_signals_change")
    plotter.plot()
