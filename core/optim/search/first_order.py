...
class FeatureSearchAntiCorrNormalized(torch.optim.Optimizer):
    def __init__(self, params, lr=1e-5, beta_utility=0.0, temp=1.0, sigma=1.0, noise_damping=True):
        names, params = zip(*params)
        self.gate_utility= None
        defaults = dict(lr=lr, beta_utility=beta_utility, temp=temp, sigma=sigma, noise_damping=noise_damping, names=names)
        super(FeatureSearchAntiCorrNormalized, self).__init__(params, defaults)
    ...
    def step(self, loss):
        for group in self.param_groups:
            for name, p in zip(reversed(group["names"]), reversed(group["params"])):
                ...
                if len(state) == 0:
                    if 'gate' in name:
                        state["avg_utility"] = torch.zeros_like(p.data).mean(dim=0).unsqueeze(1)
                        state["step"] = 0
                    state["prev_noise"] = torch.zeros_like(p.data)
                if 'gate' in name:
                    state["step"] += 1
                    ...
                    avg_utility = state["avg_utility"]
                    avg_utility.mul_(group["beta_utility"]).add_(
                        torch.mean(-p.grad.data * p.data, dim=0).unsqueeze(1), alpha=1 - group["beta_utility"]
                    )
                ...
                if self.gate_utility is not None:
                    ...
                    if len(p.data.shape) == 1:
                        # handle bias term
                        p.data.add_(noise * (1-self.gate_utility.squeeze(1)), alpha=-group["lr"])
                    else:
                        # handle weight term
