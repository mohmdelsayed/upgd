...
class SecondOrderUPGDv1Normal(torch.optim.Optimizer):
    ...
    def __init__(self, params, lr=1e-5, beta_utility=0.999, temp=5.0):
        defaults = dict(
            ...
            temp=temp,
            method_field=type(self).method.savefield,
        )
    ...
    def step(self):
        for group in self.param_groups:
            for p in group["params"]:
                ...
                p.data.add_(
                    p.grad.data
                    + noise * (1 - torch.tanh_(avg_utility / group["temp"])),
    ...
    def step(self):
        for group in self.param_groups:
            for p in group["params"]:
                ...
                p.data.add_(
                    (p.grad.data + noise)
                    * (1 - torch.tanh_(avg_utility / group["temp"])),
                    alpha=-group["lr"] / bias_correction,
                )
...
class SecondOrderUGD(torch.optim.Optimizer):
    ...
    def __init__(
        self, params, lr=1e-5, lamda=1.0, beta_utility=0.999, beta_weight=0.999
    ):
        defaults = dict(
            lr=lr,
            lamda=lamda,
