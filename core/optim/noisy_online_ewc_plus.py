...
class NoisyOnlineEWCPlus(torch.optim.Optimizer):
    def __init__(self, params, lr=1e-5, lamda=0.01, beta_weight=0.999, beta_importance=0.999, sigma=1.0, eps=1e-3):
        names, params = zip(*params)
        defaults = dict(lr=lr, lamda=lamda, beta_weight=beta_weight, beta_importance=beta_importance, sigma=sigma, eps=eps, names=names)
        super(NoisyOnlineEWCPlus, self).__init__(params, defaults)
