...
from data_generators.cifar_10 import OfflineCIFAR10
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.model = torch.nn.Sequential(
            torch.nn.Linear(32*32*3, hidden_units, True),
            torch.nn.Tanh(),
...
optimizers = [FirstOrderUPGDv1AntiCorrNormalized]
...
for optim_, color in zip(optimizers, colors):
    ...
    for step_size in step_sizes:
        ...
        for seed in list(range(n_seeds)):
            data_loader = OfflineCIFAR10(batch_size=batch_size)
            print("seed", seed)
            step = 0
